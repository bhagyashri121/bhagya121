...
public class  add extends HttpServlet {
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			String queryCheck = "SELECT name from Emp WHERE emp_id = '"
					+ emp_id + "'";
			...
			if (rs.absolute(1)) {
				connection.close();
				request.setAttribute("add_fail_message", "Employee with ID = '"
						+ emp_id + "' already exists in Database");
				request.getRequestDispatcher("/add_new.jsp").forward(request,
						response);
			}
		...
		// fail
		if (check == 0) {
			request.setAttribute("add_fail_message",
					"Failed to Add new Employee.");
			request.getRequestDispatcher("../view/add_new.jsp").forward(
					request, response);
			// response.sendRedirect("/Employee_Database/index.html");
		}

		else {
			request.setAttribute("add_success_message",
					"New Employee Added to Database.");
			request.getRequestDispatcher("./view/add_new.jsp").forward(request,
					response);
